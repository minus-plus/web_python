...
if("CHF"===c)return(a/100).toLocaleString("de-CH",{style:"currency",currency:c})+" "+c;if("CNY"===c)return(a/100).toLocaleString("zh-CH",{style:"currency",currency:c})+" "+c}else{var b=0>a?"-":"",a=Math.abs(a);if("USD"===c){var d=a%100;return b+"$"+Math.floor(a/100)+(d?"."+d:"")+" "+c}if("INR"===c)return b+"\u20b9"+Math.floor(a/100)+" "+c;if("GBP"===c){var d=Math.floor(a/100),e=a%100;return b+"\u00a3"+d+(e?"."+e:"")+" "+c}if("EUR"===c)return d=Math.floor(a/100),e=a%100,b+"\u20ac"+d+(e?","+e:"")+" "+
c;if("CHF"===c)return d=Math.floor(a/100),e=a%100,b+"CHF "+d+(e?","+e:"");if("CNY"===c)return d=Math.floor(a/100),e=a%100,b+"\u00a5"+d+(e?"."+e:"")+" "+c}return a+c}}]);angular.module("filters").filter("formatCurrency2",[function(){return function(a,c){var b;if("object"==typeof Intl&&Intl&&"function"==typeof Intl.NumberFormat)"USD"===c?b=(a/100).toLocaleString("en-US",{style:"currency",currency:c}):"INR"===c?b=(a/100).toLocaleString("en-IN",{style:"currency",currency:c}):"GBP"===c?b=(a/100).toLocaleString("en-GB",{style:"currency",currency:c}):"EUR"===c?b=(a/100).toLocaleString("at-DE",{style:"currency",currency:c}):"CHF"===c?b=(a/100).toLocaleString("de-CH",{style:"currency",
currency:c}):"CNY"===c&&(b=(a/100).toLocaleString("zh-CH",{style:"currency",currency:c})+" "+c);else{var d=0>a?"-":"",a=Math.abs(a);if("USD"===c)b=a%100,b=d+"$"+Math.floor(a/100)+(b?"."+b:"");else if("INR"===c)b=d+"\u20b9"+Math.floor(a/100);else if("GBP"===c){b=Math.floor(a/100);var e=a%100;b=d+"\u00a3"+b+(e?"."+e:"")+" "+c}else if("EUR"===c)b=Math.floor(a/100),e=a%100,b=d+"\u20ac"+b+(e?"."+e:"")+" "+c;else if("CHF"===c)b=Math.floor(a/100),e=a%100,b=d+"CHF "+b+(e?"."+e:"");else if("CNY"===c)return b=
Math.floor(a/100),e=a%100,d+"\u00a5"+b+(e?"."+e:"")+" "+c}return b?/\.00$/.test(b)?b.slice(0,-3):b:a+c}}]);angular.module("filters").filter("imageCropSize",[function(){return function(a,c){if(a)return a.match(/\/\/[^.\/]+\.(ggpht|googleusercontent)\.com\//)||a.match(/\/_ah\/img\//)?a=a.replace(/\=s\d+(-c)?/,"=s"+c+"-c"):a.match(/\/\/robohash\.org\//)&&(a=a+"?size="+c+"x"+c),a}}]).filter("imageSize",[function(){return function(a,c,b){if(a){b=0===b?void 0:b;c=0===c?void 0:c;if("number"!==typeof c&&"number"!==typeof b)return a;var d=Math.max(b,c)||b||c;if(a.match(/\/\/[^.\/]+\.(ggpht|googleusercontent)\.com\//)||
...
{lesson_key:c,group_key:d,file_format:f,description:g,perform_regrade:!!h})},getUdacityUserKeys:function(b,d,c){return a.get("/registrar/"+encodeURIComponent(b)+"/search/"+encodeURIComponent(d)+"/"+encodeURIComponent(c)).then(function(a){return a.data})},getAffiliateProfile:function(b,c){return a.get("/registrar/"+encodeURIComponent(b)+"/profiles/"+encodeURIComponent(c)).then(function(a){return a.data.profile})},deleteAffiliateProfile:function(b,c){return a.deleteRequest("/registrar/"+encodeURIComponent(b)+
...
l=b?q.trackEvent({category:"SignUp",action:"Enroll",label:b}):q.trackEvent({category:"SignUp",action:"Direct"});k=n.pushTrackingEvent("RegistrationCompleted");g.getUser();analytics.identify(a.registeredUser.key,{firstName:a.registeredUser.first_name,lastName:a.registeredUser.last_name,createdAt:new Date,email:a.registeredUser.email&&a.registeredUser.email.address},void 0,function(){analytics.track("Signed Up",{accountKey:a.registeredUser.key,category:"Sign Up"},a.segmentPageProperties,function(){return i["finally"](function(){return j["finally"](function(){return l["finally"](function(){return k["finally"](d)})})})})})},
...
e+"), but your Facebook account isn't linked. Please sign in with your Udacity account to link your Facebook account."),provider:d,email:e}:"Google"===d&&(a.initialSearch.error_existing_user={message:b.trustAsHtml("It looks like you already have a Udacity account ("+e+"), but your Google account isn't linked. Please sign in with your Udacity account to link your Google account."),provider:d,email:e}),k.path("/signin"),a._removeEmailCookie()):a.errorMessage=b.trustAsHtml("Something went wrong and we're currently looking into it.");
...
tos_accepted:!0}).then(s,function(a){u("Facebook",b.email,a)})},function(a){t("Facebook",a)})},function(a){x("Facebook",a)})};a.signUpGoogle=function(){j.getToken(!1).then(function(a){return e.put({google:{token:a.code}}).then(s,function(){return p(j.getMeResponse,5,100).then(function(a){return j.getToken(!0).then(function(b){return f.registerUser({email_address:a.email,first_name:a.given_name,last_name:a.family_name,google:{token:b.code},tos_accepted:!0}).then(s,function(b){u("Google",a.email,b)})},
function(a){x("Google",a)})},function(a){t("Google",a)})})},function(a){x("Google",a)})};r.exists("emailAddressForSignUp")&&(a.email=r.get("emailAddressForSignUp"))}]);angular.module("account").controller("viewerSettings",["$scope","currentUser","users","techDetect","$window",function(a,c,b,d){a.isBrowserIE="Explorer"===d.browserStats().browser;a.videoQualities=[{label:"Automatic",value:"default"},{label:"360p",value:"medium"},{label:"480p",value:"large"},{label:"720p",value:"hd720"},{label:"1080p",value:"hd1080"}];a.playbackRates=[{label:"0.25x",value:0.25},{label:"0.5x",value:0.5},{label:"Normal",value:1},{label:"1.5x",value:1.5},{label:"2x",value:2}];var e=c.getUser();
