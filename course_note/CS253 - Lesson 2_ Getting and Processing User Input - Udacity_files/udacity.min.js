...
angular.module("core").config(["$locationProvider","$compileProvider",function(a,c){a.html5Mode(!1);a.hashPrefix("!");c.aHrefSanitizationWhitelist(/^\s*(https?|mailto|blob):/)}]).run(["dateTime","secondsSinceEpoch","$rootScope","$location","currentUser","$window",function(a,c,b,d,e,f){a.initialize(1E3*c);var g=readCookie("ajs_user_id");b.$on("$locationChangeSuccess",function(a,d,c){function l(a){return a.replace("-"," ").replace("_"," ").replace(/\w\S*/g,function(a){return a.charAt(0).toUpperCase()+
a.substr(1).toLowerCase()})}a=!1;-1!==d.indexOf("#!")&&(-1===c.indexOf("#!")&&d.replace("#!","").replace(/\/$/,"")===c.replace(/\/$/,""))&&(a=!0);var k=d.replace("#!",""),k=k.replace(/\/$/,""),k=URI.parse(k),m=k.path.toLowerCase();""!==m&&!/^\/(me|courses|nanodegree|course|account|checkout)/.test(m)&&(a=!0);var n=e.getUser();if(n&&n._registered&&n.key){var p;p=decodeURIComponent(g);p=p.substring(1,p.length-1);p!==n.key.toString()&&(analytics.reset(),g=null);if(_.isNull(g)||"null"===g)p={firstName:n.first_name,
lastName:n.last_name},n.email&&n.email.address&&(p.email=n.email.address),analytics.identify(n.key,p),g='"'+n.key+'"'}var n=null,r=/^\/course\/.*--.*$/i,q=/^\/course\/viewer/i;p=null;var s=/^\/!c-nd\d{3}.*/i,x=/^\/course\/.*--nd\d{3}$/i;""===m?p=n="Home":"/account"===m.substring(0,8)?"/account/auth/signin"===m?p=n="Sign In":"/account/auth/signup"===m?p=n="Sign Up":(n="My Account Settings",p="My Account Settings - "+(l(m.substring(9,m.length))||"Contact Info")):"/me"===m.substring(0,3)?p=n="My Home":
"/nanodegree"===m?p=n="Nanodegrees":"/courses"===m.substring(0,8)?(n="Catalog",p="Catalog - "+l(m.substring(9,m.length))):"/checkout"===m.substring(0,9)?p=n="Checkout":x.test(m)?(n="Nanodegree Info",p="Nanodegree Info - "+m.split("--")[1]):q.test(m)?(n=/c-([^/]+)/.exec(m),p="unknown",2<=n.length&&(p=n[1]),s.test(m)?(n="My Nanodegree Classroom",p="My Nanodegree Classroom - "+p):(n="My Course Classroom",p="My Course Classroom - "+p)):r.test(m)&&(n="Course Info",p="Course Info - "+m.split("--")[1]);
var k={category:n,path:m,search:k.query,url:d},t={},s=function(a){var b={};b.conceptKey=t.exercise?a.morsel:a.exercise;b.atomKey=a.morsel;b.lessonKey=a.lesson;"nd"==t.course.toLowerCase().substring(0,2)?(b.degreeKey=a.course,b.projectKey=null,b.moduleKey=null):b.courseKey=a.course;return b};if("My Nanodegree Classroom"===n||"My Course Classroom"===n)_.each({course:/c-([^/]+)/,lesson:/l-(\d+)/,morsel:/m-(\d+)/,exercise:/e-(\d+)/},function(a,b){var d=a.exec(m);d&&d.length&&(t[b]=d[1])}),null==t.morsel?
a=!0:_.extend(k,s(t));k.referrer=d===c?document.referrer:c;b.segmentPageProperties={page:k};f.segmentPageProperties={page:k};a||analytics.page(n,p,k)})}]);angular.module("page.account",["ngRoute","core","account"]);
...
d.addSubscriptionMeta(a),d.subscription.paused_until=c.getEndDate(d.subscription))})})}}}}]);angular.module("account").factory("cancellationFlow",["modal","$q","api","$location","customers","CancellationFrame","products","recordActivity","dateTime","currentUser","$rootScope","$window",function(a,c,b,d,e,f,g,h,i,j,l,k){var m=j.fetchFullUser();return{cancelSubscription:function(d){var c=d.id,e=d.status,h=1E3*(d.trial_end||d.start),j=1E3*d.current_period_end,l=(Math.random().toString(36)+"000000000000000").slice(2,16),t=d.plan.id;"trialing"!==e&&i.calculateDaysBetweenDates(new Date(h),i.getCurrent());
f.setFlowToken(l);f.sendCancellationActivity("start");return m.then(function(){return g.get(t).then(function(a){try{return f.setCourseKey(a._course_groups[0].course_ref.key),b.get("/users/node_progress/"+f.getCourseKey()).then(function(a){f.setCompletionStats(a.data.stats);return a.data.stats})}catch(d){return f.setCourseKey("ndXXX")}})}).then(function(){return a.activate("postCancelSurvey",{subscriptionId:c,subscriptionPlanId:t,subscriptionPeriodEndDate:new Date(j),subscription:d}).then(function(){k.location.reload()}).catch(function(){f.sendCancellationActivity("abort").then(function(){f.setCurrentFrame("cancelSurvey")})})},
...
setCurrentFrame:function(a){return c=a},getFlowToken:function(){return b},setFlowToken:function(a){return b=a},getCourseKey:function(){return d},setCourseKey:function(a){return d=a},getCourseType:g,sendCancellationActivity:function(c,e,j,l){var k=b,e=e||f(),m=d,n=g(),p,r;j&&(p=j);l&&(r=l);return a.cancellationFlowStep(k,e,m,n,c,p,r)},getCompletionStats:function(){return e},setCompletionStats:function(a){e={time:Math.round(a.time/60),projects:a.projects,quizzes:a.quizzes}},sufficientTimeInClass:function(){return 30<=
...
["$animate","$document","$compile","$controller","$http","$rootScope","$q","$templateRequest","$timeout",function(a,c,b,d,e,f,g,h,i){var j=c.find("body");return new function(){var c=function(b,d){var c=b.children();return 0<c.length?a.enter(d,b,c[c.length-1]):a.enter(d,b)};this.showModal=function(e){var m=g.defer();if(!e.controller)return m.reject("No controller has been specified."),m.promise;var n=e.template,p=e.templateUrl,r=g.defer();n?r.resolve(n):p?h(p,!0).then(function(a){r.resolve(a)},function(a){r.reject(a)}):
r.reject("No template or templateUrl has been specified.");r.promise.then(function(h){var s=f.$new(),n=g.defer(),p=g.defer(),r={$scope:s,close:function(b,d){if(void 0===d||null===d)d=0;i(function(){n.resolve(b);a.leave(v).then(function(){p.resolve(b);s.$destroy();s=v=r=y=n=m=r.close=null})},d)}};e.inputs&&angular.extend(r,e.inputs);var v=b(h)(s);r.$element=v;h=d(e.controller,r);e.controllerAs&&(s[e.controllerAs]=h);e.appendElement?c(e.appendElement,v):c(j,v);var y={controller:h,scope:s,element:v,
...
6):c}}]).filter("stringDate",["$filter",function(){var a=function(a){if(!a)return a;var b=a.lastIndexOf(".");if(-1===b)return a;var d=b+7;if(d>a.length)return a;for(var e=b+1;e<d;++e)if("0">a[e]||"9"<a[e])return a;return a.substring(0,b)+a.substring(d,a.length)};return function(c){return Date.parse(a(c))}}]);angular.module("filters").filter("formatCurrency",[function(){return function(a,c){if("object"==typeof Intl&&Intl&&"function"==typeof Intl.NumberFormat){if("USD"===c)return(a/100).toLocaleString("en-US",{style:"currency",currency:c})+" "+c;if("INR"===c)return(a/100).toLocaleString("en-IN",{style:"currency",currency:c})+" "+c;if("GBP"===c)return(a/100).toLocaleString("en-GB",{style:"currency",currency:c})+" "+c;if("EUR"===c)return(a/100).toLocaleString("at-DE",{style:"currency",currency:c})+" "+c}else{var b=
0>a?"-":"",a=Math.abs(a);if("USD"===c){var d=a%100;return b+"$"+Math.floor(a/100)+(d?"."+d:"")+" "+c}if("INR"===c)return b+"\u20b9"+Math.floor(a/100)+" "+c;if("GBP"===c){var d=Math.floor(a/100),e=a%100;return b+"\u00a3"+d+(e?"."+e:"")+" "+c}if("EUR"===c)return d=Math.floor(a/100),e=a%100,b+"\u20ac"+d+(e?","+e:"")+" "+c}return a+c}}]);angular.module("filters").filter("formatCurrency2",[function(){return function(a,c){var b;if("object"==typeof Intl&&Intl&&"function"==typeof Intl.NumberFormat)"USD"===c?b=(a/100).toLocaleString("en-US",{style:"currency",currency:c}):"INR"===c?b=(a/100).toLocaleString("en-IN",{style:"currency",currency:c}):"GBP"===c?b=(a/100).toLocaleString("en-GB",{style:"currency",currency:c}):"EUR"===c&&(b=(a/100).toLocaleString("at-DE",{style:"currency",currency:c}));else{var d=0>a?"-":"",a=Math.abs(a);if("USD"===
c)b=a%100,b=d+"$"+Math.floor(a/100)+(b?"."+b:"");else if("INR"===c)b=d+"\u20b9"+Math.floor(a/100);else if("GBP"===c){b=Math.floor(a/100);var e=a%100;b=d+"\u00a3"+b+(e?"."+e:"")+" "+c}else"EUR"===c&&(b=Math.floor(a/100),e=a%100,b=d+"\u20ac"+b+(e?"."+e:"")+" "+c)}return b?/\.00$/.test(b)?b.slice(0,-3):b:a+c}}]);angular.module("filters").filter("imageCropSize",[function(){return function(a,c){if(a)return a.match(/\/\/[^.\/]+\.(ggpht|googleusercontent)\.com\//)||a.match(/\/_ah\/img\//)?a=a.replace(/\=s\d+(-c)?/,"=s"+c+"-c"):a.match(/\/\/robohash\.org\//)&&(a=a+"?size="+c+"x"+c),a}}]).filter("imageSize",[function(){return function(a,c,b){if(a){b=0===b?void 0:b;c=0===c?void 0:c;if("number"!==typeof c&&"number"!==typeof b)return a;var d=Math.max(b,c)||b||c;if(a.match(/\/\/[^.\/]+\.(ggpht|googleusercontent)\.com\//)||
...
function(b){b=b.status;a.buttonDisabled=!1;a.errorMessage=400===b?c.trustAsHtml("We couldn't find an account with that email."):409===b?c.trustAsHtml("We have recently sent a reset email to that address. It may have been marked as spam."):c.trustAsHtml("Something went wrong and we were unable to send your reset email.")}):(a.fieldErrors=e.errorText,a.buttonDisabled=!1)}}}]);angular.module("account").controller("signInForm",["$scope","$q","$sce","session","users","currentUser","groups","validator","google","facebook","$location","safelyRedirect",function(a,c,b,d,e,f,g,h,i,j,l,k){a.hideSocialSignIn=!1;var m=a.initialSearch.next;f.userIsLoggedIn()&&k(m);if(a.initialSearch.error_existing_user&&(e=a.initialSearch.error_existing_user,e.message&&(a.errorMessage=e.message),e.email&&(a.email=e.email),"Facebook"===e.provider||"Google"===e.provider))a.hideSocialSignIn=!0;a.showTopBox=
a.initialSearch.showMessage;var n=function(b){try{a.signedInUserAccountKey=b.data.account.key,a.signedInUserEmail=b.config.data.udacity.username}catch(d){console.log("Cannot retrieve accountKey and username"),a.signedInUserAccountKey=null,a.signedInUserEmail=null}var b=a.initialSearch.enroll,e=a.initialSearch.enroll_state||"enrolled",h=a.initialSearch["join-group"],b=b?f.enroll(b,e,!0):c.when(!0),i=h?f.fetchFullUser().then(function(){return g.addMember(h,f.get("key"))}):c.when(!0);return b.then(function(){return i.then(function(){f.getUser();
analytics.identify(a.signedInUserAccountKey,{email:a.signedInUserEmail},void 0,function(){analytics.track("Signed In",{accountKey:a.signedInUserAccountKey,category:"Sign In"},a.segmentPageProperties,function(){analytics.user()&&analytics.alias(a.signedInUserAccountKey,analytics.user().anonymousId(),a.segmentPageProperties,function(){k(m)})})})})})},p=function(d){a.errorMessage=b.trustAsHtml("We were unable to validate your account.");return c.reject(d)},r=function(d){if(403===d.data.status)a.errorMessage=
b.trustAsHtml('Your social account isn\'t connected yet, you can sign in with an existing account, or <a href="#!/signup">Sign Up</a> to get started.');else{if(502===d.data.status)return p(d);a.errorMessage=b.trustAsHtml("Something went wrong and we're currently looking into it.")}return c.reject(d)};a.signInEmail=function(){a.buttonDisabled=!0;a.fieldErrors=null;a.email=$("#email").val();a.password=$("#password").val();var e=h.check({email:{pretty:"email address",value:a.email,validators:[{type:"required",
chained:[{type:"email"}]}]},password:{pretty:"password",value:a.password,validators:[{type:"required"}]}});e.valid?d.put({udacity:{username:a.email,password:a.password}}).then(n,function(d){a.buttonDisabled=!1;a.errorMessage=b.trustAsHtml("The email or password you entered is invalid");return c.reject(d)}):(a.fieldErrors=e.errorText,a.buttonDisabled=!1)};a.signInFacebook=function(){return j.getAuthResponse().then(function(a){return d.put({facebook:{signed_request:a.signedRequest}}).then(n,r)},p)};
a.signInGoogle=function(){i.getToken(!1).then(function(a){return d.put({google:{token:a.code}}).then(n,r)},p)};a.hasLongCookieString=function(){return 3E3<document.cookie.length}}]);angular.module("account").controller("signUpForm",["$scope","$q","$sce","$timeout","session","users","currentUser","groups","validator","google","facebook","$location","safelyRedirect","googleTagManager","retry","cookieJar","universalAnalytics",function(a,c,b,d,e,f,g,h,i,j,l,k,m,n,p,r,q){a.signUpFormFillTracked=!1;a.$watchGroup(["email","password","firstName","lastName"],function(b){!a.signUpFormFillTracked&&_.any(b,function(a){return void 0!==a})&&analytics.track("Sign Up Started",{category:"Sign Up"},
a.segmentPageProperties,function(){a.signUpFormFillTracked=!0})});a._removeEmailCookie=function(){r.exists("emailAddressForSignUp")&&r.remove("emailAddressForSignUp",{path:"/"})};var s=function(b){a.registeredUser=b;var d=function(){var b=a.initialSearch.next;b?m(b):m("/")},b=a.initialSearch.enroll,e=a.initialSearch.enroll_state||"enrolled",f=a.initialSearch["join-group"],i=b?g.enroll(b,e,!0):c.when(!0),j=f?g.fetchFullUser().then(function(){return h.addMember(f,g.get("key"))}):c.when(!0),k,l;a._removeEmailCookie();
k=b?q.trackEvent({category:"SignUp",action:"Enroll",label:b}):q.trackEvent({category:"SignUp",action:"Direct"});l=n.pushTrackingEvent("RegistrationCompleted");g.getUser();analytics.identify(a.registeredUser.key,{firstName:a.registeredUser.first_name,lastName:a.registeredUser.last_name,createdAt:new Date,email:a.registeredUser.email&&a.registeredUser.email.address},void 0,function(){analytics.track("Signed Up",{accountKey:a.registeredUser.key,category:"Sign Up"},a.segmentPageProperties,function(){return i["finally"](function(){return j["finally"](function(){return k["finally"](function(){return l["finally"](d)})})})})})},
x=function(d,e){a.errorMessage=b.trustAsHtml("We were unable to validate your "+d+" account. ");return c.reject(e)},t=function(d,e){a.errorMessage=b.trustAsHtml("We were unable to access your "+d+" account.");return c.reject(e)},u=function(d,e,f){409===f.data.status?("Udacity"===d?a.initialSearch.error_existing_user={message:b.trustAsHtml("This account ("+e+") already exists, please Sign In to continue."),provider:"udacity",email:e}:"Facebook"===d?a.initialSearch.error_existing_user={message:b.trustAsHtml("It looks like you already have a Udacity account ("+
e+"), but your Facebook account isn't linked. Please sign in to link your Facebook account."),provider:d,email:e}:"Google"===d&&(a.initialSearch.error_existing_user={message:b.trustAsHtml("It looks like you already have a Udacity account ("+e+"), but your Google account isn't linked. Please sign in to link your Google account."),provider:d,email:e}),k.path("/signin"),a._removeEmailCookie()):a.errorMessage=b.trustAsHtml("Something went wrong and we're currently looking into it.");a.buttonDisabled=
...
function(a){return a.course_ref&&a.course_ref.key})}function u(b,c){var e={product_keys:c.productKeys};b&&b.id&&(e.stripe_token=b.id);c.coupon&&c.coupon.valid&&(e.coupon_id=c.coupon.key);var f,g;c.customer&&(g=_.find(c.customer.subscriptions,function(a){return a.plan.metadata.sku===c.product._course_groups[0].course_ref.key&&"free"===a.plan.metadata.variant.toLowerCase()}));g&&(f=d.post("/customers/me/subscriptions/"+g.id+"/change_plan",{new_product_key:c.product.key}));if(c.endTrial){var h=_.find(c.customer.subscriptions,
function(a){return a.plan.id===c.product.key}).id;f=h?d.post("/customers/me",{stripe_token:e.stripe_token}).then(function(){return d.deleteRequest("/customers/me/subscriptions/"+h+"/trial")}):a.reject({data:{}})}else f||(f=d.post("/customers/me/purchases",e));f=f.then(function(a){return a.data.purchase},function(b){c.generalPaymentFormError=b.data.error?x(b.data):"We are having difficulties processing your request. If this error keeps appearing please contact payment-support@udacity.com.";b.data.was_charged&&
alert("Error - your card was charged successfully but an error occured.Please contact us at payment-support@udacity.com immediately and we'll get this sorted out. Your spot is reserved. Please do NOT pay again.");return a.reject(b)});f.then(function(){return c.course.isNanodegree&&c.course.shouldSeeOnboarding?i.setOnboardingFlag("me",c.course.key):a.when()});return f}function v(a,b,c,d,f){var g={step:1,cc_prefilled:!!a.customerCard,sku:null,product_key:null,name:null,price:null,currency:"USD",trial_period_days:null,
coupon:null,coupon_valid:!1,coupon_description:null,coupon_redeem_by:null,discount_amount:0,discount_type:null,enrolled_nanodegrees:e.getEnrolledNanodegreeKeys().toString(),enrolled_courses:e.getEnrolledCourseKeys().toString(),price_before_discount:null,funnel_id:y,account_key:q.account.key,is_email_verified:q.account.email&&q.account.email._verified};a.product&&(g.product_key=a.product.key,g.name=a.product.name,g.price=a.product.amount/100,g.price_usd=g.price,g.currency=a.product.currency,g.trial_period_days=
a.product.trial_period_days,g.price_before_discount=g.price,g.value=g.price,g.nd_variant=a.product.variant,g.variant=a.product.variant,g.price_usd=g.price,"INR"==g.currency&&(g.value*=0.015,g.price_usd=g.value));a.course&&(g.category="Degree"===a.course.courseType?"Nanodegree "+(g.nd_variant||"STANDARD"):"Career"===a.course.courseType?"Career Advisory":"Paid Course",g.sku=a.course.key,g.label=g.sku);a.coupon&&a.coupon.key&&(g.coupon=a.coupon.key,g.coupon_description=a.coupon.description||null,g.coupon_redeem_by=
a.coupon.redeem_by||null,g.discount_type=a.coupon.duration||null,a.coupon.valid&&(g.coupon_valid=a.coupon.valid,g.discount_amount=g.price_before_discount?(a.product.amount-a.coupon.discounted_price)/100:0,g.price=a.coupon.discounted_price/100));"Added Product"===c?g.quantity=1:"Completed Order"===c&&(g=_.extend(g,{orderId:b,total:g.price,revenue:g.price,shipping:0,tax:0,discount:g.discount_amount,coupon:g.coupon,currency:g.currency,products:[{id:g.product_key,sku:g.sku,name:g.name,price:g.price,quantity:1,
category:g.category,variant:g.variant,coupon:g.coupon}]}));d&&(g.start_time=d);f&&(g.end_time=new Date,g.time_on_page_sec=(g.end_time-g.start_time)/1E3);return g}var y=s();return[{label:"Checkout",view:"/templates/angular/payment/payment_form.html",onStart:function(b,e){e.funnel_id=y;var i=[f.get(e.productKeys[0]),g.get("me")];if(h){var j=d.get("/coupons/"+h+"/compute_discount",{product_key:e.productKeys[0]}),j=j.then(function(a){e.coupon=a.data.coupon;a.data.coupon.valid&&(e.coupon.discounted_price=
a.data.result)},function(){e.coupon={valid:!1};return null});i.push(j)}a.all(i).then(function(a){e.product=a[0];var b=_.first(e.product._course_groups).course_ref.key;e.customer=a[1];e.customerCard=c.getDefaultCard(e.customer);e.cardIsLoaded=!0;e.uriEncodedCourseKey=window.encodeURIComponent(t(e.product));k.get(b).then(function(a){var b=a.catalog_entry._image?a.catalog_entry._image.serving_url:null,c=a.overview_details?a.overview_details.should_see_onboarding:!1;e.confirmationStepLabel={Career:"Start program",
Degree:"Start Nanodegree",Course:"Start course"}[a.getCourseType()];e.course={key:a.key,imageUrl:b,isNanodegree:a.isNanodegree(),isCareer:a.isCareer(),courseType:a.getCourseType(),shouldSeeOnboarding:c};p.pushTrackingEvent("checkoutStart",{productName:e.product.name,productCurrency:e.product.currency,productPrice:e.product.amount,orderNodeKey:e.course.key,isNanodegree:a.isNanodegree,isCareer:a.isCareer,getCourseType:a.getCourseType});a=v(e,null,"Viewed Checkout Step");e.segmentProps=a;e.segmentProps.start_time=
new Date;analytics.track("Viewed Checkout Step",a,q.segmentPageProperties);analytics.track("Added Product",a,q.segmentPageProperties);if(e.coupon&&e.coupon.no_credit_card_required)return u(!1,e).then(function(){q.location.href="/me"});if(e.product.autoenroll&&!e.endTrial)return u(!1,e).then(function(){q.location.href="/course/viewer#!/c-"+q.encodeURIComponent(e.course.key)});var d;if(e.product.variant&&"free"===e.product.variant.toLowerCase())if(e.customer&&(d=_.find(e.customer.subscriptions,function(a){return a.plan.metadata.sku===
e.product._course_groups[0].course_ref.key&&a.plan.metadata.variant&&"free"!==a.plan.metadata.variant.toLowerCase()})),d)console.log("user is enrolled in non-free variant");else return u(!1,e).then(function(){q.location.href="/me"})});e.endTrial?e.inTrialPeriod=!1:(a=e.product.trial_period_days,e.inTrialPeriod=0<a,e.trialPeriodStarts=new Date(Date.now()+a*m),a=new Date(e.trialPeriodStarts),b=a.getMonth(),a.setMonth(a.getMonth()+1),a.getMonth()!=(b+1)%12&&a.setDate(0),e.secondPaymentDue=a)})},onJump:function(a){return a.paymentConfirmed?
!1:!a.paymentConfirmed},onSubmit:function(b,d){if(d.customerCard)return u(!1,d);var e=b.creditCard||{},f=c.checkValidity(e);d.creditCardErrors=f.errorText;if(!f.valid)return!1;f=(d.product&&d.product.name)+", "+t(d.product);return c.createPaymentToken(f,e).then(function(a){return u(a,d)},function(b){d.generalPaymentFormError=x(b.error);return a.reject(b)})}},{getLabel:function(a){return a.confirmationStepLabel},view:"/templates/angular/payment/payment_confirmation.html",onStart:function(a,b){b.paymentConfirmed=
!0;b.payment=a;var c=b.product&&(b.product.amount||0)/100,d=null;try{d=_.find(a.customer.subscriptions,function(a){return a.plan.id===b.product.key}).id}catch(e){l.error("Failed to get Transaction Id",e)}r.trackTransaction({transactionId:d,transactionTotal:c,transactionAffiliation:"Udacity",transactionProducts:[{name:b.product&&b.product.name||"Missing product name, FILL IN IN CONTENT EDITOR ASAP FOR "+_.first(t(b.product)),sku:"DD44",price:c,quantity:1,category:_.first(t(b.product))}]});p.pushTrackingEvent("CompletedOrder",
{transactionVal:c,transactionOrderId:d,transactionPromoCode:h,orderNodeKey:b.course.key,isNanodegree:b.course.isNanodegree});b.abandonedCheckout=!1;analytics.track("Completed Checkout Step",v(b,d,"Completed Checkout Step",b.segmentProps.start_time,!0),q.segmentPageProperties);analytics.track("Completed Order",v(b,d,"Completed Order",b.segmentProps.start_time,!0),q.segmentPageProperties)}}]}]);angular.module("payment").directive("numbersOnly",function(){return{require:"ngModel",link:function(a,c,b,d){d.$parsers.push(function(a){if(void 0==a)return"";var b=a.replace(/[^0-9]/g,"");b!=a&&(d.$setViewValue(b),d.$render());return b})}}});angular.module("payment").factory("paymentFlow",["$q","paymentState","$location",function(a,c,b){return{begin:function(a){a=c.startFlow(a);if(a.onStart)a.onStart(null,c.getCurrentFlowData());b.path("/"+c.getCurrentStep().number.toString())},submitStep:function(d){var e=a.defer();(d=c.getCurrentStep().onSubmit(d,c.getCurrentFlowData()))?a.when(d).then(function(a){var d=c.goToNextStep();if(d.onStart)d.onStart(a,c.getCurrentFlowData());e.resolve();b.path("/"+d.number.toString())},function(){e.resolve()}):
...
g.pending[e]&&g.pending[e].readyNotification(g.run);if(null!==b)g.pending=_.without(g.pending,b),b["finally"](g.run),g.running.push(b),c(b.start);else break}}};return g};d=a.make;return a}]);angular.module("user").factory("currentUser",["$log","$q","$rootScope","users","basicUser","nodes","nodeUtility","userUtility","cohortsGateway","universalAnalytics","publicCatalog","$http","applicationEnvironment",function(a,c,b,d,e,f,g,h,i,j,l,k,m){var n=angular.copy(e),p=null,r=null,q={get:function(a){return n[a]},getUser:function(){return n},getFullName:function(){return h.getFullName(n)},fetchIsStaff:function(){return q.fetchFullUser().then(function(a){if(a._memberships)return 0<_.filter(a._memberships,
function(a){return-1<a.group_ref.key.indexOf("staff")}).length})},getEnrolledCourses:function(a){a=a||["enrolled","paused"];return _.chain(q.get("_enrollments")).filter(function(b){return b&&_.contains(a,b.state)}).pluck("node_key").value()},getEnrolledCourseKeys:function(a){a=a||["enrolled","paused"];return _.filter(q.getEnrolledCourses(a),function(a){return 0!==a.indexOf("nd")})},getEnrolledNanodegreeKeys:function(a){a=a||["enrolled","paused"];return _.filter(q.getEnrolledCourses(a),function(a){return 0===
a.indexOf("nd")})},getEnrolledCareerKeys:function(a){a=a||["enrolled","paused"];return _.filter(q.getEnrolledCourses(a),function(a){return 0===a.indexOf("ca")})},getEnrolledProductVariantFor:function(a){var b=q.getUser()._enrollments.filter(function(b){return b.node_key===a});return 0<b.length?b[0].product_variant:null},fetchGraduatedNanodegreeKeys:function(){return q.fetchCertificates().then(function(a){return _.chain(a).map(function(a){return a.node_ref.key}).filter(function(a){return 0===a.indexOf("nd")}).value()})},
fetchGraduatedCourseKeys:function(){return q.fetchCertificates().then(function(a){return _.map(a,function(a){return a.node_ref.key})})},fetchGraduatedCareerKeys:function(){return q.fetchCertificates().then(function(a){return _.chain(a).map(function(a){return a.node_ref.key}).filter(function(a){return 0===a.indexOf("ca")}).value()})},fetchEnrolledAndGraduatedCourseKeys:function(a){return q.fetchFullUser().then(function(){return q.fetchGraduatedCourseKeys()}).then(function(b){return _.union(q.getEnrolledCourses(a),
b)})},fetchInferredCreatedAt:function(){return q.fetchFullUser().then(function(a){var b,a=_.chain(a._memberships).pluck("creation_time").compact().map(function(a){return new Date(a)}).value();a.length&&(b=_.min(a).toISOString());return b})},fetchEnrolledAndGraduatedNanodegreeKeys:function(){return q.fetchGraduatedNanodegreeKeys().then(function(a){return _.union(q.getEnrolledNanodegreeKeys(),a)})},fetchEnrolledAndGraduatedCareerKeys:function(){return q.fetchGraduatedCareerKeys().then(function(a){return _.union(q.getEnrolledCareerKeys(),
a)})},getGroupKeys:function(){return _.chain(q.get("_memberships")).filter(function(a){return a.group_ref&&"Group"===a.group_ref.ref}).map(function(a){return a.group_ref.key}).value()},fetchStudyGroups:function(){return d.getStudyGroups("me")},fetchActiveExperiments:function(a){return d.fetchActiveExperiments("me",a)},getEmail:function(){return h.getEmail(n)},setUser:function(a){angular.copy(angular.copy(a),n)},fetchFullUser:function(a){var b=a?{cacheBust:Math.random()}:{};if(a)return d.get("me",
b).then(function(a){q.setUser(a);return a});p||(p=d.get("me",b).then(function(a){q.setUser(a);return a}));return p},fetchCertificates:function(){return n._certificates?c.when(n._certificates):d.getCertificates("me").then(function(a){return n._certificates=a})},checkNewHomeBanner:function(){return d.checkNewHomeBanner("me")},setNewHomeBanner:function(){return d.setNewHomeBanner("me")},setFinalStepsEmailSent:function(a){return d.setFinalStepsEmailSent("me",a)},sendFinalStepsEmail:function(a,b){return d.sendFinalStepsEmail("me",
a,b)},setFiftyBackBannerFlag:function(a){return d.setFiftyBackBannerFlag("me",a)},checkReviewForNd:function(a,b){var c=m.isLocal()?"http://dockerhost:9000/api/v1":"https://rocky-basin-6876.herokuapp.com/api/v1";return k({url:c+"/reviews",type:"GET",params:{userKey:a,nodeKey:b}}).then(function(a){return a.data&&a.data._id}).catch(function(a){console.error(a)})},fetchCohorts:function(){r||(r=q.fetchFullUser().then(function(a){return c.all(_.map(a._cohort_keys,function(a){return i.get(a)}))},function(b){a.error(b)}));
return r},userIsLoggedIn:function(){return n._registered},isEnrolledIn:function(a){return _.some(n._enrollments,function(b){return b.node_key===a&&"enrolled"===b.state})},isEnrolledForCreditIn:function(a){var b=["151101088","150207572","151148782","151059885","151254701"];return void 0!==_.find(n._enrollments,function(c){var d=_.pluck(c.group_refs,"key");return c.node_key===a&&"enrolled"===c.state&&0<_.intersection(d,b).length})},isCourseEmailSubscribed:function(a){return 0<_.filter(n._enrollments,
function(b){return b.node_key===a&&b.email_ok}).length},enroll:function(a,c,e){c=c||"enrolled";return d.setEnrollments("me",[a],c).then(function(a){n._enrollments=n._enrollments||[];_.each(a,function(a){var b=_.find(n._enrollments,function(b){return b.node_key===a.node_key});b?n._enrollments[_.indexOf(n._enrollments,b)]=a:n._enrollments.push(a)});if(1===a.length){b.$broadcast("modifiedEnrollment",a);var d=c.slice(0,1).toUpperCase()+c.slice(1,-2),d=j.trackEvent({category:"Enrollment",action:d,label:a[0].node_key});
return e?d.then(function(){return a},function(){console.warn("Failed to push enrollment to Universal Analytics");return a}):a}return a})},changeEnrollmentSettings:function(a,b){return d.changeEnrollmentSettings(a,"me",b)},getEnrollmentsWithState:function(a){return _.filter(n._enrollments,function(b){return b.state===a})},getTags:function(){return d.getTags("/users/me/tags")},setTag:function(a,b){var e=q.get("tags"),a=a.toLowerCase(),b=b.toLowerCase();if(_.findWhere(e,{namespace:a,tag:b}))return c.when(e);
var f=e.concat([{namespace:a,tag:b}]);n.tags=f;return d.put({key:q.get("key"),tags:f}).catch(function(a){n.tags=e;return c.reject(a)})},removeTag:function(a,b){var e=q.get("tags");if(!_.findWhere(e,{namespace:a,tag:b}))return c.when(e);var f=_.reject(e,function(c){return c.namespace===a&&c.tag===b});n.tags=f;return d.put({key:q.get("key"),tags:f}).catch(function(a){n.tags=e;return c.reject(a)})},isNanodegreeStudent:function(){return 0<_.size(q.getEnrolledNanodegreeKeys(["enrolled","unenrolled","paused"]))},
checkIfProfilesEnabled:function(){var a=q.getEnrolledNanodegreeKeys(["enrolled","pre-enrolled","unenrolled","paused"]);return 0===_.size(a)?c.when(!1):l.fetchCatalogAsMapOfNodeKeysToNode().then(function(b){return _.some(a,function(a){return b[a]&&b[a].enables_profiles})})},isUserEnrolledInCourseThroughND:function(a){return a.related_degree_keys||a.catalog_entry&&a.catalog_entry.related_degree_keys?q.fetchEnrolledAndGraduatedNanodegreeKeys().then(function(b){return 0<_.size(_.intersection(a.related_degree_keys?
a.related_degree_keys:a.catalog_entry.related_degree_keys,b))}):-1<a.key.indexOf("-nd")?c.when(!0):c.when(!1)},fetchIsReviewer:function(){return d.fetchIsUserReviewer(n.key)},copyProgressToUdacityTwoDotO:function(){return d.copyProgressToUdacityTwoDotO("me")},fetchShouldSeeUpgradeBanner:function(){return c.all({isStaff:q.fetchIsStaff(),isReviewer:q.fetchIsReviewer()}).then(function(a){var b=_.last(q.get("key")),c=["ca001","ca002","ca003","nd001b"];return!_.some(q.get("_enrollments"),function(a){return-1<
c.indexOf(a.node_key)})&&(a.isStaff||a.isReviewer||"0"===b||"7"===b||"1"===b)})}};return q}]);angular.module("user").directive("userMenu",["currentUser","session",function(a,c){return{restrict:"A",link:function(b){b.isLoggedIn=a.userIsLoggedIn;b.user=a.getUser;b.isNanodegreeStudent=a.isNanodegreeStudent();b.isProfilesEnabled=!1;a.checkIfProfilesEnabled().then(function(a){b.isProfilesEnabled=a});var d=[/\/account/,/\/me/,/\/checkout/];b.logOut=function(){analytics.track("Signed Out",{category:"Sign Out",account_key:window.account.key},b.segmentPageProperties,function(){c.terminate().then(function(){analytics.reset();
...
"&cache_bust="+j};a.userIsLoggedIn=u.userIsLoggedIn();a.canEditNotes=y;a.course=null;a.lessons=null;a.lesson=null;a.morsels=null;a.morsel=null;a.tidbit=null;var w=b.path().match("/"+m.course+"([^/]+)"),z=b.path().match("/"+m.lesson+"([^/]+)"),w=w?w[1]:null,z=z?z[1]:null;a.courseKeyFromUrl=w;a.lessonKeyFromUrl=z;w||(window.location.href="/");s.addEventListener("sessionExpired",function(){window.location.href="/account/auth#!/signin?next=%2Fcourse%2Fviewer%23!"+encodeURIComponent(b.path())});x&&(window.location.href=
"/account/auth#!/signin?next=%2Fcourse%2Fviewer%23!"+encodeURIComponent(b.path()));C.fetchFreeTrialComplete(w).then(function(a){a.trialing&&a.projectComplete&&C.installPaywallModal(a,w,z)});c.addEventListener("lessonPermissionDenied",function(){window.location.href="/course/"+w});c.addEventListener("courseChange",function(a){if((!a.lessons||0===a.lessons.length)&&!a.lastLessonVisited&&!d.lessonKey)window.location.href="/course/"+w}).addEventListener("coursePermissionDenied",function(){d.lessonKey||
(window.location.href="/course/"+w)});c.addEventListener("lessonChange",function(a){if(!a.morsels||0===a.morsels.length)window.location.href="/course/"+w});a.isViewerLoading=!0;c.addEventListener("courseChange",function(){n.loadLibraries(["codemirror","mathquill"]).then(function(){},function(){I.error("Classroom failed to load a required library");alert("Failed to load some required parts of the course! Please refresh the page. If this issue persists, please contact support@udacity.com. Thanks!")})}).addEventListener("morselChange",
function(){a.isViewerLoading=!1});c.addEventListener("courseChange",function(b){a.course=b.course;a.lessons=b.lessons;a.enrollment=_.find(u.get("_enrollments"),function(a){return a.node_key===b.course.key});a.premiumAccess=v.hasPermissionTo(a.course,"view-paid-supplemental-media")||a.enrollment&&(v.isCurrentEnrolled(a.enrollment,b.course)||v.isGraduated(a.enrollment,b.course));H.set({isPaid:a.premiumAccess?"yes":"no"})},{replayLast:!0,removeOnDestructionOf:a}).addEventListener("lessonChange",function(b){a.lesson=
b.lesson;a.morsels=b.morsels},{replayLast:!0,removeOnDestructionOf:a}).addEventListener("morselChange",function(b){a.morsel=b;E(b)},{replayLast:!0}).addEventListener("tidbitChange",function(b){(a.tidbit=b.tidbit)&&E(b.tidbit)},{replayLast:!0});c.addEventListener("pendingLessonChange",function(){a.morsels=null;a.morsel=null;a.tidbit=null});a.$on("$routeChangeSuccess",function(b,f,g){if(b=!d.morselKey)if(!g||!f)b=!1;else{var h=g.pathParams,f=f.pathParams;if(_.keys(h).length===_.keys(f).length)b=!1;
else{var i=!0;_.each(f,function(a,b){h[b]!==a&&(i=!1)});b=i}}b&&window.history.back();d.courseKey&&d.courseKey!==w&&window.location.reload();g&&J.trackVirtualPageView();if((null===a.course||a.course.key!==d.courseKey)&&"none"!==d.courseKey)D(),c.addEventListener("courseChange",function(b){d.lessonKey||(b.lastLessonVisited?a.goToLesson(b.lastLessonVisited.key):a.goToLesson(b.lessons[0].key))},{removeAfterFired:!0}),c.changeCourse(d.courseKey);if((null===a.lesson||a.lesson.key!==d.lessonKey)&&d.lessonKey)D(),
...
b.containingExercise&&(b=b.containingExercise);b=b.indexInLesson;b<c-1?a.goToMorsel(a.morsels[b+1]):(h.pauseVideo(),a.course?u.userIsLoggedIn()?a.lessons&&a.lesson.key===a.lessons[a.lessons.length-1].key?(a.enrollment=_.find(u.get("_enrollments"),function(b){return b.node_key===a.course.key}),t.activate("endOfCourse",{courseIsComplete:a.course.catalog_entry.complete,courseKey:a.course.key,isPaidEnrolled:v.isCurrentEnrolled(a.enrollment,a.course),isGraduated:v.isGraduated(a.enrollment,a.course)})):
t.activate("endOfLesson",{lessonKey:a.lesson.key,userIsLoggedIn:u.userIsLoggedIn()}):t.activate("endOfLessonSignInGate",{lessonKey:a.lesson.key,userIsLoggedIn:u.userIsLoggedIn()}):t.activate("endOfIsolatedContent"))};a.goToPreviousMorsel=function(){var b=a.morsel;b.containingExercise&&(b=b.containingExercise);b=b.indexInLesson;0<b?a.goToMorsel(a.morsels[b-1]):console.error("Can't go to previous; at beginning of lesson")};a.goToLesson=function(e){if(!a.lesson||e!==a.lesson.key)c.announcePendingLessonChange(),
f(function(){b.path("/"+m.course+d.courseKey+"/"+m.lesson+e)})};a.goToNextLesson=function(){if(a.lessons){var b=a.lessons.length,c=_.find(a.lessons,function(b){return b.key===a.lesson.key}).indexInCourse;0<=c&&c<b-1?a.goToLesson(a.lessons[c+1].key):console.error("Can't go to next; at end of course!")}else console.error("Can't go to next lesson, we're not in a course!")};a.os=q.os(!1);var K=G.lessonIsCareerProject;a.shouldShowLesson=function(b){if(b=K(b))b=a.enrollment?(b=a.enrollment.product_variant)?
